...
새 이슈 작업은 **반드시 아래 순서대로** 진행한다. 이 사이클은 Claude가 흐름을 제어하는 워크플로우다 — 각 단계가 끝나면 결과를 보고하고 **다음 단계를 제안**하되, **사용자 승인 없이 자동으로 다음 단계로 넘어가지 않는다.**

| # | 단계 | 호출 | 종류 | 역할 |
|---|------|------|------|------|
| 1 | 시나리오 | `/test-scenarios N` | skill | 시그니처 + 테스트 시나리오 확정 |
| 2 | Red | `/tdd-red N` | skill | 실패하는 테스트 작성 (구현 금지) |
| 3 | Green | `/tdd-green N` | skill | 최소 구현으로 테스트 전체 통과 (테스트 수정 금지) |
| 4 | AC 검증 | `/ac-verifier N` | agent | AC 충족을 독립 검증 — **테스트 통과 ≠ AC 충족** |
| 5 | Refactor | `/tdd-refactor N` | skill | 구조 개선, 테스트 깨지면 즉시 롤백 |
| 6 | 보안 점검 | `/security-review N` | skill | 타입(`tsc`)·의존성·`.env` 점검 (커밋 게이트) |
| 7 | 통합 | — | — | commit & push → PR `--base feature/<spec>` → squash merge → 이슈 클로즈 |

### 흐름 제어 규칙

- **인간 승인 게이트**: 각 단계 사이에는 사용자 승인 게이트가 있다. 단계 완료 후 "다음은 N단계(`/...`)입니다. 진행할까요?" 형태로 제안하고 **멈춘다.**
- **순서 강제**: 단계를 건너뛰거나 앞당기지 않는다. AC 검증(4)은 Green(3) 이후, Refactor(5)는 AC 검증 통과 이후에만 진행한다.
- **롤백 우선**: Refactor 중 테스트가 깨지면 추가 수정으로 살리려 하지 말고 즉시 직전 상태로 롤백한다.
- **테스트/구현 경계**: Red는 `src/`를 건드리지 않고, Green·Refactor는 테스트 파일을 수정하지 않는다.
- **이슈 의존성**: 선행 이슈에 의존하면, 그 선행 이슈가 머지된 `feature/<spec>` 브랜치에서 분기한다.
- **진입 감지**: 사용자가 "이슈 N 시작", "다음 이슈" 등으로 작업을 열면 현재 사이클 위치를 파악해 알맞은 단계를 제안한다.
